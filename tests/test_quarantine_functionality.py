...
class TestQuarantine(unittest.TestCase):
    ...
    def test_add_to_quarantine_orders_correctly(self):
        ...
        q.add(time=10, value="A", horizon=5, step=1)   # release = 15
        q.add(time=5, value="B", horizon=20, step=1)  # release = 25
        q.add(time=12, value="C", horizon=3, step=1)   # release = 15
    ...
    def test_pop_returns_empty_when_no_item_ready(self):
        ...
        q.add(time=10, value="A", horizon=5, step=1)    # release = 15
        q.add(time=20, value="B", horizon=10, step=1)   # release = 30
    ...
    def test_pop_returns_multiple_items(self):
        ...
        q.add(time=10, value="A", horizon=5, step=1)   # 15
        q.add(time=11, value="B", horizon=4, step=1)   # 15
        q.add(time=20, value="C", horizon=10, step=1)  # 30
    ...
    def test_pop_clears_all_items_if_all_ready(self):
        ...
        q.add(time=1, value="A", horizon=1, step=1)  # 2
        q.add(time=2, value="B", horizon=1, step=1)  # 3
    ...
    def test_pop_multiple_calls_progressively(self):
        ...
        q.add(time=10, value="A", horizon=5, step=1)   # 15
        q.add(time=12, value="B", horizon=3, step=1)   # 15
        q.add(time=20, value="C", horizon=10, step=1)  # 30
